...
module.exports = async (client) => {
    client.on("interactionCreate", async (interaction) => {
        ...
        if (!member.voice.channel)
            return interaction?.reply({
                content: `<a:no:921989165242003476> **Please join a Voice Channel first!**`,
                ephemeral: true
            })      
            
        var player = client.manager.players.get(interaction?.guild.id);
        if (interaction?.customId != "Join" && interaction?.customId != "Leave" && (!player || !player.queue || !player?.queue?.current))
            return interaction?.reply({content: "<a:no:921989165242003476> Nothing Playing yet", ephemeral: true})
        ...
        if (player && member.voice.channel.id !== player.voiceChannel)
            return interaction?.reply({
                content: `<a:no:921989165242003476> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`,
                ephemeral: true
            })
        ...
        if(player && interaction?.customId != `Join` && interaction?.customId != `Lyrics` && dj) {
                return interaction?.reply({embeds: [new MessageEmbed()
                  ...
                  .setTitle(`<a:no:921989165242003476> **You are not a DJ and not the Song Requester!**`)
        ...
        switch(interaction?.customId){
            ...
            case "Autoplay": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.get(`autoplay`) ? `<a:yes2:921988805442043925> **Enabled Autoplay**`: `<a:no:921989165242003476> **Disabled Autoplay**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = await generateQueueEmbed(client, guild.id)
            ...
            case "Song": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.trackRepeat ? `<a:yes2:921988805442043925> **Enabled Song Loop**`: `<a:no:921989165242003476> **Disabled Song Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = await generateQueueEmbed(client, guild.id)
            ...
            case "Queue": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.queueRepeat ? `<a:yes2:921988805442043925> **Enabled Queue Loop**`: `<a:no:921989165242003476> **Disabled Queue Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = await generateQueueEmbed(client, guild.id)
    ...
    client.on("messageCreate", async message => {
        ...
        if (prefixRegex.test(message.content)) {
            ...
            if (cmd || cmd.length === 0) return// message.reply("<a:no:921989165242003476> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => null);}catch(e){ }}, 3000)})
            ...
            {
                return// message.reply("<a:no:921989165242003476> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => null);}catch(e){ }}, 3000)})
            }
        }
        ...
        if (!channel) return message.reply("<a:no:921989165242003476> **Please join a Voice Channel first!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => null);}catch(e){ }}, 5000)})
        ...
        if (player && channel.id !== player.voiceChannel) return message.reply(`<a:no:921989165242003476> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`).then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => null);}catch(e){ }}, 3000)})
...
async function generateQueueEmbed(client, guildId, leave){
    ...
    var embeds = [
      ...
      new MessageEmbed()
        ...
        .setImage(guild.banner ? guild.bannerURL({size: 4096}) : "https://media.discordapp.net/attachments/873381531769528353/935007071349375026/kk.gif")
        .setTitle(`Start Listening to Music, by connecting to a Voice Channel and sending either the **SONG LINK** or **SONG NAME** in this Channel!`)
        .setDescription(`> *I support <a:YouTube:922768065446424586> Youtube, <a:spotify:922768224750272582> Spotify, <a:Soundcloud:923829456768991272> Soundcloud and direct MP3 Links!*`)
    ]
