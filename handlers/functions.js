...
async function GetUser(message, arg){
  var errormessage = "<a:no:921989165242003476> I failed finding that User...";
...
async function GetRole(message, arg){
  var errormessage = "<a:no:921989165242003476> I failed finding that Role...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("CLIENT IS NOT DEFINED")
    if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
  ...
  return new Promise(async (resolve, reject) => {
    ...
    if(!user && args[0] && args[0].length == 18) {
      ...
      for await (const g of guilds){
        var members = g.members.cache.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777);
      ...
      if (data.rosterstyle == "1") {
        ...
        for (let i = 0; i < memberarray.length; i += 20) {
          ...
          if (!guildData[pre].showallroles || memberarray.length < 20)
            try { 
              rosterembed.addField(`**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).length}]__**`, role.members.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024) + `${role.members.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).length - 20 > 0 ? `\n${guildData[pre].rosteremoji} ***\`${role.members.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).length - 20}\` other Members have this Role ...***`: ""}`.substring(0, 1024), guildData[pre].inline)
              break;
            } catch (e) {
              console.error(e)
            }
            ...
            try {
              rosterembed.addField(i < 20 ? `**__${role.name.toUpperCase()} [${role.members.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).length}]__**` : `\u200b`, role.members.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).length == 0 ? "> No one has this Role" : thearray.slice(i, i + 20).join("\n").substring(0, leftnum <= 1024 ? leftnum : 1024), guildData[pre].inline)
            } catch (e) {
              console.error(e)
            }
        }
        ...
        await msgs.forEach(async msg => {
            ...
            if(msg.type == "DEFAULT"){
              ...
              if (msg.reactions && msg.reactions.cache.size > 0){
                ...
                for await (const reaction of msg.reactions.cache.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777)){                      
 ...
 */
  ...
  let button_back = new MessageButton().setStyle('SUCCESS').setCustomId('1').setEmoji("975381288267677716").setLabel("Back")
  let button_home = new MessageButton().setStyle('DANGER').setCustomId('2').setEmoji("🏠").setLabel("Home")
  let button_forward = new MessageButton().setStyle('SUCCESS').setCustomId('3').setEmoji('975381256193843240').setLabel("Forward")
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author?.id)
        return b?.reply({content: `<a:no:921989165242003476> **Only the one who typed ${prefix}help is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
...
async function swap_pages_data(client, message, description, TITLE, T_cmd = "Unkown") {
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author?.id)
        return b?.reply({content: `<a:no:921989165242003476> **Only the one who typed the cmd is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
          collector.resetTimer();
          //b?.reply("***Swapping a PAGE FORWARD***, *please wait 2 Seconds for the next Input*", true)
...
async function check_voice_channels(client) {
  ...
  for await (const guild of guilds) {
    for await (const [key, value] of Object.entries(rawData.find(d => d.ID == guild.id)?.data).filter(([key, value]) => value && value.channel && value.channel.length > 6)) {
      ...
      try{
        let members = channel.members.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777);
...
async function create_transcript(message, client, msglimit) {
  ...
  let msgs = messageCollection.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).reverse(); //reverse the array to have it listed like the discord chat
