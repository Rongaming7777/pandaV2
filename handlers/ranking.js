...
const RonCanvasApi = require("./CanvasApi")
...
async function run (client) {
    ...
    client.on("ready", async () => {
        ...
        client.guilds.cache.each(async g => {
            ...
            rankingAllCache.set(g.id, allPoints.filter(p => p.data?.guild === g.id).map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777.data))
...
async function cachePointsAll(client, GuildId) {
    ...
    if(!rankingAllCache.get(GuildId)) {
        ...
        rankingAllCache.set(GuildId, await client.points.all().then(db => db.filter(p => p.data?.guild === GuildId).map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777.data)))
    } else {
...
async function messageCreate(client, message, guild_settings, setups) {
    try{
        ...
        async function LEVELUP(thekey) {
            ...
            const { buffer } = await RonCanvasApi.getLevelUpCard({
        ...
        async function rank(the_rankuser, type = "text") {
            ...
            try {
                ...
                const { buffer: rankBuffer } = await RonCanvasApi.getRankCard({
                    xp_data: xp_data,
                    rankuser: rankuser,
                    status: status,
        ...
        async function newleaderboard(type = "text") {
            ...
            // Get the buffer from the api / Handler File
            const { buffer: buffer1 } = await RonCanvasApi.getLeaderboardCard({
                type: "text",
            ...
            // Get the buffer from the api / Handler File
            const { buffer: buffer2 } = await RonCanvasApi.getLeaderboardCard({
                type: "voice",
