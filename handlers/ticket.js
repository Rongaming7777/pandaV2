...
module.exports = (client, preindex) => {
  ...
  client.on("interactionCreate", async (interaction) => {
      ...
      try {
        ...
        if (data) {
          if (data.state != "closed") {
            if(!interaction.replied) await interaction.reply({ content: `<a:no:921989165242003476> **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            else await interaction.editReply({ content: `<a:no:921989165242003476> **You already have an Ticket!** <#${ticketchannel.id}>`, ephemeral: true }).catch(console.warn);
            return
          }
        }
      } catch {
     ...
     * CREATE THE CHANNEL
     */
    if(!interaction.replied) await interaction.reply({ content: `<a:loding:925692920781234196> **Creating your Ticket...** (Usually takes 0-2 Seconds)`, ephemeral: true }).catch(console.warn);
    else await interaction.editReply({ content: `<a:loding:925692920781234196> **Creating your Ticket...** (Usually takes 0-2 Seconds)`, ephemeral: true }).catch(console.warn);
