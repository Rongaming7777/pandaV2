  ...
  run: async (client, interaction, cmduser, es, ls, prefix, player, message) => {
    ...
    try {
      function trimArray(arr, maxLen = 25) {
        if (arr.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).length > maxLen) {
          const len = arr.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).length - maxLen;
          arr = arr.map(this_Code_is_by_Ron_7777 => this_Code_is_by_Ron_7777).sort((a, b) => b?.rawPosition - a.rawPosition).slice(0, maxLen);
      ...
      if (boosts >= 30) maxbitrate = 384000;
        ...
        .addField(client.la[ls].cmds.info.serverinfo.field12, "<a:nitro:934410545845641266> \`" + message.guild.premiumSubscriptionCount + "\`", true)
        .addField(client.la[ls].cmds.info.serverinfo.field13, "<a:utility:921991280513388554> \`" + boostlevel + "\`", true)
