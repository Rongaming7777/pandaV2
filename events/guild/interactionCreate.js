      ...
      if(command.parameters.type == "music"){
        ...
        if(channel && !mechannel) {
          if(!channel?.permissionsFor(message.guild?.me)?.has(Permissions.FLAGS.CONNECT)) 
            return interaction.reply({ ephemeral: true, content: "<a:no:921989165242003476> **I'm missing the Permission to Connect to your Voice-Channel!**"}).catch(() => null);
          if(!channel?.permissionsFor(message.guild?.me)?.has(Permissions.FLAGS.SPEAK)) 
            return interaction.reply({ ephemeral: true, content: "<a:no:921989165242003476> **I'm missing the Permission to Speak in your Voice-Channel!**"}).catch(() => null);
        }
        ...
        if(player && player.queue && player.queue.current && command.parameters.check_dj){
          if(check_if_dj(client, interaction?.member, player.queue.current)) {
            return interaction?.reply({embeds: [new MessageEmbed()
              ...
              .setTitle(` <: no: 932825003441946685 > ** You are not a DJ and not the Song Requester! ** `)
