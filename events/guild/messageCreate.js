...
module.exports = async (client, message) => {
  try {
    ...
    if(!message.guild.me?.permissions?.has(Permissions.FLAGS.EMBED_LINKS))
      return message.reply(`<a:no:921989165242003476> **I am missing the Permission to EMBED LINKS (Sending Embeds)**`).catch(console.error)
    if(!message.guild.me?.permissions?.has(Permissions.FLAGS.ADD_REACTIONS))
      return message.reply(`<a:no:921989165242003476> **I am missing the Permission to ADD REACTIONS**`).catch(console.error)
    ...
    if (botchannel && botchannel.length > 0 && !botchannel?.includes(message.channel.id) && !message.member?.permissions?.has("ADMINISTRATOR")) {
      ...
      try {
        message.react("932825003441946685").catch(console.error)
      } catch {}
      not_allowed = true;
    ...
    if (command && !customcmd) {
      ...
      if(musicData && musicData.channel && musicData.channel == message.channel.id){
        return message.reply("<a:no:921989165242003476> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => null);}catch(e){ }}, 3000)}).catch(console.error)
      }
