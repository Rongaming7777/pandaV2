    ...
    try {
      ...
      async function first_layer(){
        ...
        let MenuEmbed = new MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.getAuthor('Youtube-Poster', 'https://cdn.discordapp.com/emojis/935018819360014347.png?size=128', 'https://discord.gg/zG8yUPhuxw'))
        ...
        collector.on('end', collected => {
          menumsg.edit({embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)], components: [], content: `${collected && collected.first() && collected.first().values ? `<a:yes2:921988805442043925> **Selected: \`${collected && collected.first() ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`})
        });
      }
      ...
      async function handle_the_picks(optionhandletype, SetupNumber, menuoptiondata) {
        switch (optionhandletype) {
          ...
          var tempmsg = await message.reply({embeds: [new Discord.MessageEmbed()
            ...
            .setColor(es.color)
            .setDescription(`Example:\nhttps://www.youtube.com/channel/UC1AgotpFHNhzolUtAjgZqQ`)
            .setFooter(client.getFooter(es))]
          })
          await tempmsg.channel.awaitMessages({filter: m => m.author.id === message.author?.id,
              max: 1,
            ...
            .then(async collected => {
              ...
              if(msg && msg.content ){
                if((msg.content.length > 0 && msg.content.length < 50) &&!msg.content.toLowerCase().includes("youtube") && (!msg.content.toLowerCase().includes("channel") || !msg.content.toLowerCase().includes("c")))
                  return message.reply("YOU DID NOT SEND A VALID YOUTUBE CHANNEL\nNote, such links doesn't work: `https://youtube.com/Ron` / `https://youtube.com/c/Ron`\nIt must be something like this: `https://www.youtube.com/channel/UC1AgotpFHNhzolUtAjPgZ`")
          ...
          case "Edit Youtube Channel":{
            ...
            let MenuEmbed = new MessageEmbed()
              .setColor(es.color)
              .setAuthor('Youtube-Poster', 'https://cdn.discordapp.com/emojis/935018819360014347.png?size=128', 'https://discord.gg/zG8yUPhuxw')
