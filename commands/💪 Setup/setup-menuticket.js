    ...
    try {
      ...
      async function first_layer() {
        ...
        let MenuEmbed = new Discord.MessageEmbed()
          .setColor(es.color)
          .setAuthor(client.getAuthor('Menu Ticket Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/envelope_2709-fe0f.png', 'https://discord.gg/zG8yUPhuxw'))
        ...
        switch (optionhandletype) {
          case "Ticket Claim System":{
            ...
            */
            ...
            async function third_layer(SetupNumber) {
              ...
              let MenuEmbed = new Discord.MessageEmbed()
                .setColor(es.color)
                .setAuthor(SetupNumber + " Ticket Setup", "https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/twitter/282/incoming-envelope_1f4e8.png", "https://discord.gg/zG8yUPhuxw")
              ...
              collector.on('end', collected => {
                menumsg.edit({
                  embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                  components: [],
                  content: `${collected && collected.first() && collected.first().values ? `<a:yes2:921988805442043925> **Selected: \`${collected && collected?.first()?.values?.[0] ? collected.first().values[0] : "Nothing"}\`**` : "❌ **NOTHING SELECTED - CANCELLED**" }`
                })
              });
            }
          ...
          case "Send the Config	Message": {
            ...
            if (!data || data.length < 1) {
              return message.reply("<a:no:921989165242003476> **You need to add at least 1 Open-Ticket-Option**")
            }
          ...
          case "Edit Ticket Option": {
            ...
            let data = await theDB.get(`${message.guild.id}.${pre}.data`, true);
            if (!data || data.length < 1) {
              return message.reply("<a:no:921989165242003476> **There are no Open-Ticket-Options to remove**")
            }
            let embed = new MessageEmbed()
              .setColor(es.color)
              .setThumbnail(es.thumb ? es.footericon && (es.footericon.includes("http://") || es.footericon.includes("https://")) ? es.footericon : client.user.displayAvatarURL() : null)
                ...
                async function handle_the_picks3(optionhandletype) {
                  ...
                  switch (optionhandletype) {
                    ...
                    case `Change Open Category`:{
                      ...
                      return message.reply("<a:no:921989165242003476> **Invalid Category-ID added**")
                    }break;
            ...
            collector.on('end', collected => {
              menumsg.edit({
                embeds: [menumsg.embeds[0].setDescription(`~~${menumsg.embeds[0].description}~~`)],
                components: [],
                content: `<a:yes2:921988805442043925> **Selected: \`${collected.size > 0 ? collected.first().values[0] : "NOTHING"}\`**`
              })
            });
          }
          break;
