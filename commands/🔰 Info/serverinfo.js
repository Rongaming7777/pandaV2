...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player) => {
    ...
    try {
      ...
      embed.addField("**<a:arrow:923076073942564864> Rules Channel:**", `${message.guild.rulesChannel ? `<#${message.guild.rulesChannelId}>`: "<a:no:921989165242003476> \`No Channel\`"}`, true)
      embed.addField("**<a:arrow:923076073942564864> Public Updates Channel:**", `${message.guild.publicUpdatesChannel ? `<#${message.guild.publicUpdatesChannelId}>`: "<a:no:921989165242003476> \`No Channel\`"}`, true)
      embed.addField("**<a:arrow:923076073942564864> AFK Channel:**", `${message.guild.afkChannel ? `<#${message.guild.afkChannelId}>`: "<a:no:921989165242003476> \`No Channel\`"}`, true)

      embed.addField("**<a:arrow:923076073942564864> NSFW Level:**", `\`${message.guild.nsfwLevel}\``, true)
      embed.addField("**<a:arrow:923076073942564864> Verifcation Level:**", `\`${message.guild.verificationLevel}\``, true)
      embed.addField("**<a:arrow:923076073942564864> Explicit Content Filter:**", `\`${message.guild.explicitContentFilter}\``, true)
      ...
      embed.addField(client.la[ls].cmds.info.serverinfo.field12, "<a:nitro:934410545845641266> \`" + message.guild.premiumSubscriptionCount + "\`", true)

      embed.addField(client.la[ls].cmds.info.serverinfo.field13, `<a:utility:921991280513388554> \`${boostlevel}\``, true)
      embed.addField(client.la[ls].cmds.info.serverinfo.field14, "👾 \`" + maxbitrate + " kbps\`", true)
      if(boosts >= 14){
          embed.addField(`**<a:arrow:923076073942564864> Vanity:**`, `${message.guild.vanityURLCode ? `https://discord.gg/${message.guild.vanityURLCode}` : "<a:no:921989165242003476> No Vanity-Invite"}`)
      }
      ...
      if(message.guild.banner) {
        ...
        .setTitle(`**<a:arrow:923076073942564864> SERVER BANNER:**`)
        ...
        let embed2 = new Discord.MessageEmbed()
        .setTitle(`**<a:arrow:923076073942564864> SERVER DISCOVERY SPLASH:**`)
