...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    try {
      ...
      var button_forward = new MessageButton().setStyle('PRIMARY').setCustomId('notes_forwards').setEmoji('975381256193843240').setLabel("Forwards")
      var button_back = new MessageButton().setStyle('PRIMARY').setCustomId('notes_backwards').setEmoji("975381288267677716").setLabel("Backwards")
      ...
      var button_disable = new MessageButton().setStyle('SECONDARY').setCustomId('notes_disable').setLabel('Stop the Buttons').setEmoji("932825003441946685");
      ...
      if(!notes || notes.length == 0){
          ...
          .setTitle(`<a:no:921989165242003476> No Notes created yet`)
      ...
      try{
        if(b?.user.id !== message.author?.id)
          ...
          if(b?.customId == "notes_disable") {
            ...
            var buttonRow2 = new MessageActionRow().addComponents([button_create, button_edit, button_Delete, button_disable])
            allbuttons = [buttonRow1, buttonRow2]
            await b?.reply("<a:no:921989165242003476> **Disabled the Buttons**", true);
          ...
          if(b?.customId == "notes_jump") {
            ...
            await mmmmm.channel.awaitMessages({filter: m=>m.author.id == cmduser.id, max: 1, time: 180e3}).then(async collected => {
              ...
              if(Page < 0 || Page > embeds.length - 1){
                return message.reply("<a:no:921989165242003476> **Value out of Range!**").then(msg=>{
                  setTimeout(()=>{
                    try{msg.delete()}catch(e){console.error(e)}
                  }, 2500)
                }).catch(e=>{
