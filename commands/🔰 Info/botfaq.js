...
module.exports = {
    ...
    let es = client.settings.get(message.guild.id, "embed");let ls = client.settings.get(message.guild.id, "language")
		...
		try{
      let rondc = client.guilds.cache.get("920944376983740446")
      let ronmembers = await rondc.members.fetch().catch(() => {});
      let partnercount = ronmembers.filter(m => m.roles.cache.has("823150244509515807"))
      ...
      .setColor(es.color)
      .setAuthor(client.la[ls].cmds.info.botfaq.menuembed.title, client.user.displayAvatarURL(), "https://dsc.gg/bot-support.invite")
