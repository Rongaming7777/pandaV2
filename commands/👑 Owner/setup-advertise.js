  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    try {
      ...
      async function first_layer(){
        let menuoptions = [
          {
            ...
            description: `${client.ad.enabled? "Disables the Ads from Ron" : "EnableRonds from Ron"}`,
            emoji: clienRonabled? emoji?.react.ERROR : emoji?.react.SUCCESS
          },
          {
            value: "Settings",
        ...
        .setColor(es.color)
        .setAuthor(client.getFooter('Advertising Setup', 'https://emojipedia-us.s3.dualstack.us-west-1.amazonaws.com/thumbs/120/apple/285/prohibited_1f6.ab.png',  'https://discord.gg/zG8yUPhuxw'))
        ...
        client.on('interactionCreate', async (menu) => {
          if (menu?.message.id === menumsg.id) {
            if (menu?.user.id === cmduser.id) {
              if(used1) return menu?.reply({content : `<a:no:921989165242003476> You already selected something, this Selection is now disabled!`}, {ephermal : true});
              menuselection(menu);
            }
            else menu?.reply({content : `<a:no:921989165242003476> You are not allowed to do that! Only: <@${cmduser.id}>`}, {ephermal : true});
          }
        });
      }
