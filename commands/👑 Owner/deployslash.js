    ...
    if (message.author?.id != "442355791412854784")
      return message.channel.send({
        embeds: [new MessageEmbed()
          ...
          .setTitle("Only Ron is allowed to deploy the SLASH-COMMANDS")
          .setDescription(`Go to the [Discord-Server](https://discord.gg/zG8yUPhuxw), open a Ticket and ask for it!`)
        ]
      });
    try {
      ...
      if(loadSlashsGlobal){
        let themsg = await message.reply(`<a:loding:925692920781234196> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
        ...
        let guild = client.guilds.cache.get(guildId);
        let themsg = await message.reply(`<a:loding:925692920781234196> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
