      ...
      if(channel.permissionOverwrites.cache.size < 1){
        if((users && users.length > 0) || (roles && roles.length > 0)){
          ...
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:yes2:921988805442043925> **Successfully locked \`${channel.name}\` for ${users.length} Users and ${roles.length} Roles**`)
          ]});
        } else {
          await channel.permissionOverwrites.set([{
            id: message.guild.roles.everyone.id,
        ...
        if((users && users.length > 0) || (roles && roles.length > 0)){
          ...
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:yes2:921988805442043925> **Successfully locked \`${channel.name}\` for ${users ? users.length : 0} Users and ${roles ? roles.length : 0} Roles**`)
          ]});
        } else {
          if(channel.permissionOverwrites.cache.filter(permission => permission.allow.toArray().includes("SEND_MESSAGES")).size < 1)
            return message.reply({embeds :[new MessageEmbed()
