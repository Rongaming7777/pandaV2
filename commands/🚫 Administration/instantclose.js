...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
    ...
    try {
      ...
      let button_ticket_verify = new MessageButton().setStyle('SUCCESS').setCustomId('ticket_verify').setLabel("Verify this Step").setEmoji("921988805442043925")
      message.reply({
          content: `<@${buttonuser.id}>`,
          embeds: [new Discord.MessageEmbed()
          ...
          collector.on('end', collected => {
              if (!edited) {
                  ...
                  msg.edit({
                      content: `<@${buttonuser.id}>`,
                      embeds: [endedembed],
                      components: [new MessageActionRow().addComponents(button_ticket_verify.setDisabled(true).setLabel("FAILED TO VERIFY").setEmoji("932825003441946685").setStyle('DANGER'))]
                  }).catch((e) => {
                      console.error(e)
                  });
              }
