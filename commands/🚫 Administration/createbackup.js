...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
        ...
        if(!message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)){
            return message.reply("<a:no:921989165242003476> **I am missing the ADMINISTRATOR Permission!**")
        }
        ...
        if(owner.id != cmduser.id) {
            return message.reply(`<a:no:921989165242003476> **You need to be the Owner of this Server!**`)
        }
        ...
        message.channel.send({
            ...
            collector.on('collect', button => {
                if (button?.user.id === cmduser.id) {
                    collector.stop();
                    button?.reply({content: `<a:loding:925692920781234196> **Now saving the Backup!**\nThis could take up to 2 Minutes (belongs to your data amount)`}).catch(() => null)
                    // Create the backup
