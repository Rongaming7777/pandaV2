...
let buttonEvent = async (interaction) => {
  ...
  if (interaction?.message.id === msg.id) {
    if (interaction?.user.id === message.member.id) {
      ...
      await interaction?.reply({content : `<a:no:921989165242003476> You are not allowed to do that! Only: <@${message.author?.id}>`, ephemeral : true}); //ephemeral message
    }
  }
}
...
let embedEditing = async(button) => {
    ...
    if(id.startsWith(`builder`)) {
        ...
        button?.message.edit({content : `<a:loding:925692920781234196> **Please send me Your Input now!**`,
