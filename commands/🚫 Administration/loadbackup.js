...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix, player, es, ls, GuildSettings) => {
        ...
        if(!server) return message.reply(`<a:no:921989165242003476> **You forgot to add from which Server i should load the Backup in here**\n> Type: \`${prefix}loadbackup <ServerId> <BackupId>\``)
        if(message.guild.me && !message.guild.me.permissions.has(Discord.Permissions.FLAGS.ADMINISTRATOR)){
            return message.reply(`<a:no:921989165242003476> **I am missing the ADMINISTRATOR Permission in ${message.guild.name}!**`)
        }
        if(server.ownerId != cmduser.id || message.guild.ownerId != cmduser.id) {
            return message.reply(`<a:no:921989165242003476> **You need to be Owner in both Servers!**`)
        }
        ...
        if(!backups || backups.length == 0) {
            return message.reply(`<a:no:921989165242003476> **There are no Backups in ${server.name}**\nCreate one with: \`${prefix}createbackup\``)
        }
        if(!args[1]) return message.reply(`<a:no:921989165242003476> **You forgot to add the Backup Id**\n> Type: \`${prefix}loadbackup <ServerId> <BackupId>\``)
        if(isNaN(args[1]) || Number(args[1]) < 1 || Number(args[1]) > 5) return message.reply(`<a:no:921989165242003476> **The Backup Id Must be a Number between 1 and 5**\n> Type: \`${prefix}loadbackup <ServerId> <BackupId>\``)
        if(backups.length < Number(args[1])) {
            return message.reply(`<a:no:921989165242003476> **The Provided Backup Id does not exist!**\n> Type: \`${prefix}listbackups\` to see all Backups`)
        }
        let backupData = backups[Number(args[1]) - 1];
        if(!backupData) {
            return message.reply(`<a:no:921989165242003476> **The __Provided__ Backup Id does not exist!**\n> Type: \`${prefix}listbackups\` to see all Backups`)
        }
        ...
        message.channel.send({
            ...
            collector.on('collect', button => {
                if (button?.user.id === cmduser.id) {
                    collector.stop();
                    button?.reply({content: `<a:loding:925692920781234196> Now loading the Backup from \`${server.name}\` to \`${message.guild.name}\`!`}).catch(() => null)
                    // Create the backup
